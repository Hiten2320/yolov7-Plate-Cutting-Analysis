...
def ROI_Checker(det_arr, dobj, _del = False):
    ...
    if (np.where(det_arr[:,4] == dobj)[0]).size!=0:
        for i in range(len(np.where(det_arr[:,4] == dobj)[0])):
            ...
            # if _del:
            #     try:
            #         det_arr = np.delete(det_arr, np.where(det_arr[:,4] == dobj)[0][i],axis = 0) if r!=1.0 else det_arr
            #     except:
            #         det_arr = np.delete(det_arr, np.where(det_arr[:,4] == dobj)[0][i-1],axis = 0) if r!=1.0 else det_arr
            #         pass
        #det_arr = np.delete(det_arr, delrows ,axis = 0) if len(delrows)!=0 else det_arr
        #print("After: ",det_arr)
...
def detect(source, Camname):
    global status_time
    source, weights = source, "sheet.pt"
    sort_tracker = tracker_initialize()
    # filename = source.split('.')[0]
    # vidsave = videosave(source, resize=0.5)
    vidsave = videosave(source, f"lodingoutput.mp4")
